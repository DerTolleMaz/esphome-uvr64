...
void DLBusSensor::setup() {
  ...
  ESP_LOGI(TAG, "DLBusSensor setup complete on pin %d", pin_);
}
...
void IRAM_ATTR DLBusSensor::isr(void *arg) {
  ...
  if (duration < 1000) return;  // Software-Entprellung: min. Zeit 1ms
  self->last_edge_ = now;
...
void DLBusSensor::parse_frame_() {
  ...
  const uint32_t BIN_WIDTH = 2000;
  int histogram[BIN_COUNT] = {0};
  int overflows = 0;
  for (int i = 0; i < bit_index_; i++) {
    int bin = timings_[i] / BIN_WIDTH;
    if (bin >= BIN_COUNT)
      overflows++;
    else
      histogram[bin]++;
  ...
  for (int i = 0; i < bit_index_; i++) {
    if (timings_[i] < 6000 || timings_[i] > 14000) {
